from tests.aux.functions import branin4_function
...
def branin4_cli(path):
    ...
    while p.poll() is None:  # Check if the process is still running
        ...
        if "End of BaCO" in request:  # This means that BaCO ended
            print(request)
            break
        ...
        p.stdin.write(str_to_baco)
        p.stdin.flush()  # Reply to BaCO with all the evaluations
        i += 1
