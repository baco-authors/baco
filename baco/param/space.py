...
from baco.param.parameters import Parameter, RealParameter, IntegerParameter, OrdinalParameter, CategoricalParameter, PermutationParameter
...
class Space:
    def __init__(self, settings: Dict):
        ...
        In addition there is time_stamps which contains the evaluation timestamps for each evaluation.

        The configurations X are stored in their "base form" where categoricals and permutations are stored as numeric indices. There are three other
        representations, "String representation" used to print and check constraints, "parametrized representation" used in the GP model, and
        [0-1] representation used on opentuner.
        ...
        max_number_of_predictions = settings["max_number_of_predictions"]
        ...
        self.evaluations_per_optimization_iteration = settings["evaluations_per_iteration"]
        self.normalize_inputs = settings["normalize_inputs"]
        self.normalize_priors = True

        baco_mode = settings["baco_mode"]["mode"]

        if self.conditional_space:
            cot_order, tree_orders = self.get_cot_order()
            self.chain_of_trees = ChainOfTrees(cot_order)
    ...
    def create_chain_of_trees(self, tree_orders:List[str]):
        """
        ...
        for tree_order in tree_orders:
            ...
            self.build_tree(tree, tree.get_root(), torch.Tensor([[torch.nan]*len(tree_order)]), tree_order, -1)
            ...
            sys.stdout.write_to_logfile(
                f"time to calculate probabiities: {time.time() - propagate_probabilities_time_start}"
            )
            ...
            if level == len(tree_order) - 2:
                for idx, child_value in enumerate(possible_child_values):
                    child_node = Node(
                        ...
                        child_parameter.val_indices[child_value],
                    )
    ...
    def get_cot_order(self) -> Tuple[List[int], List[List[int]]]:
        """
        Creates dependecy graph to find a topological sorting of the parameters.
        """
    ...
    def get_default_configuration(self):
        """
        Returns:
            - the default configuration from the input parameters space if complete and feasible.
        """
        ...
        if None in default_configuration:
            ...
            return None
        default_configuration = self.convert([default_configuration], "original", "internal")
    ...
    def convert(
        ...
        # output = [[(parameters[column].convert(data[row][column], from_type, to_type) if not np.isnan(data[row][column]) else torch.nan) for column in columns] for row in rows]
    ...
    def evaluate(self, configurations:torch.Tensor) -> List[bool]:
        """
        ...
        return constraints.evaluate_constraints(self.constraints, transformed_configurations)
    ...
    def filter_out_previously_run(
        ...
        """
        accepted_indices = [
            i for i, l in enumerate([self.get_unique_hash_string_from_values(x) for x in to_filter]) if not l in previously_run
        ]
    ...
    def run_configurations_client_server(
        ...
        Input:
            ...
            - run_directory: the working directory where BaCO is run (this is specified in the json file).
        """
        ...
        for conf_index in range(configurations.shape[0]):
            line = sys.stdin.readline()
            sys.stdout.write(line)
            # same as .split(','') but don't split on commas within paranthesis to allow for permutations
    ...
    def run_configurations_with_black_box_function(
        ...
        if self.enable_feasible_predictor:
            metrics_array = torch.tensor(objective_values)[:,:-1]
            feasible_array = torch.tensor(objective_values)[:,-1].to(dtype=torch.bool)
        else:
            metrics_array = torch.tensor(objective_values)
            feasible_array = torch.Tensor()

        timestamp_array = torch.tensor(timestamps)
        data_array = DataArray(configurations, metrics_array, timestamp_array, feasible_array)
    ...
    def run_configurations(
        ...
        """
        Run a set of configurations in one of BaCOs modes.

        Input:
            - baco_mode: which BaCO mode to run as.
            - configurations: a list of configurations (dict).
            - beginning_of_time: time from the beginning of the BaCO design space exploration.
            - run_directory: the working directory where BaCO is run (this is specified in the json file).
            - black_box_function: objective function being optimized.
            - beginning_of_time: time from the beginning of the BaCO design space exploration.
            - exhaustive_search_data_array: array containing all data from the exhaustive search.
            - exhaustive_search_fast_addressing_of_data_array: dict containing the indices of each configuration in the data array.
            - run_directory: the directory where BaCO is running.
            - number_of_cpus: number of cpus to use in parallel.
        Returns:
        ...
        if baco_mode == "default":
            ...
            print("Unrecognized baco mode:", baco_mode)
            raise SystemExit
        return data_array
