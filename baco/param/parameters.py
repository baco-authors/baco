...
class IntegerParameter(Parameter):
    ...
    def __init__(
        ...
        self.val_indices = dict(zip(self.values, list(range(len(self.values)))))
        self.transform = transform

        if isinstance(probability_distribution, str):
            self.distribution_name = probability_distribution
            if self.distribution_name == "uniform":
                self.distribution = np.ones(len(self.values))/len(self.values)
        else:
            self.distribution_name = "custom_distribution"
            self.distribution = list(
                np.array(probability_distribution) / np.sum(probability_distribution)
            )
    ...
    def pdf(self, x: int) -> float:
        ...
        Returns:
            - the probability of x
        """
        return self.distribution[self.val_indices[x]]
...
class CategoricalParameter(Parameter):
    ...
    def __init__(
        ...
        self.values = list(range(len(values)))
        ...
        if isinstance(probability_distribution, str):
            self.distribution_name = probability_distribution
            if (self.distribution_name == "uniform"):
                self.distribution = np.ones(self.get_size())/ self.get_size()
        else:
            self.distribution_name = "custom_distribution"
            self.distribution = probability_distribution
        assert self.distribution_name in [
    ...
    def sample(self, size=1, unifor=False) -> torch.Tensor:
        """
    ...
    def pdf(self, x_idx: int) -> float:
        """
        Compute the probability of a given X under the prior distribution of the parameter.
        Returns:
        - the probability of X
        """
        return self.distribution[x_idx]
    ...
    def get_original_string(self, idx_value: int) -> str:
        return self.string_values[int(idx_value)].encode()
...
class PermutationParameter(Parameter):
    ...
    def __init__(
        ...
        self.values = [x for x in range(len(self.permutation_values))]
        default_index = None
        if default:
            default_index = self.permutation_values.index(tuple(default))
        Parameter.__init__(self, name, default_index, constraints, dependencies)

        self.parametrization = parametrization.lower()
        self.distribution = np.ones(self.get_size()) / self.get_size()
        self.val_indices = {v: i for i, v in enumerate(self.permutation_values)}
    ...
    def parametrize(self, data: List[int]) -> Tuple[List[str], List[List[float]]]:
        ...
        :returns: a list of of names for the new variables
        ...
        if self.parametrization == "spearman":
            return (
                ...
                [
                    [self.permutation_values[int(d)][i] / self.n_elements for d in data]
            ...
            return (
                ...
                [
                    [self.permutation_values[int(d)][i] == j for d in data]
                    for i in range(self.n_elements)
                    for j in range(self.n_elements)
                ],
            )
    ...
    def string_to_int(self, str: str) -> int:
        if str[0] == "(":
            return self.get_int_value(tuple(int(x) for x in str[1:-1].split(",")))
        else:
            return self.get_int_value(tuple(int(x) for x in str.split(",")))
