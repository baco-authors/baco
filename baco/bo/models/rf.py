...
class RFRegressionModel(RandomForestRegressor):
    ...
    def get_node_visits(self, tree: Any, x_leaves:np.ndarray):
        """
    ...
    def get_node_bounds(self, samples:List[int], data_array:np.ndarray, threshold:float):
        """
        ...
        Input:
            ...
            - data_array: list containing the values of one parameter for all of the samples from the data.
...
class RFClassificationModel(RandomForestClassifier):
    ...
    def __init__(
        ...
        """
        Fit a Random Forest classsification model.

        Input:
            - settings:
            - param_space: parameter space object for the current application.
            - X: input traianing data
