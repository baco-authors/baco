...
def ei(
    ...
    **kwargs,
    ...
    if settings["log_transform_output"]:
        best_values = torch.log10(best_values)
    ...
    objective_ei = (
        (normalized_best_values - f_means - xi) * torch.tensor(stats.norm.cdf(v)) +
        f_stds * torch.tensor(stats.norm.pdf(v))
    )
    scalarized_ei = (objective_ei @ objective_weights)
    if verbose and objective_ei.shape[0] == 1:
        sys.stdout.write_to_logfile(
            f"f*:{' '.join(str(x.item()) for x in normalized_best_values)}  mu:{' '.join(str(x.item()) for x in f_means.squeeze(0))}  sigma:{' '.join(str(x.item()) for x in f_stds.squeeze(0))} "
            + f"feasibility:{feasibility_indicator.item()} p1:{' '.join(str(x.item()) for x in ((normalized_best_values - f_means - xi) * stats.norm.cdf(v)).squeeze(0))} "
            + f"p2:{' '.join(str(x.item()) for x in (f_stds * stats.norm.pdf(v)).squeeze(0))} alpha:{' '.join(str(x.item()) for x in objective_ei.squeeze(0))} w:{' '.join(str(x) for x in objective_weights)}\n"
        )

    # minus because the the local search is a minimizing the negative expected improvement
    acq_val = (
        -1
        * scalarized_ei
        * feasibility_indicator
