...
def get_prior(
    ...
    """
    probabilities = torch.ones(X.shape[0], dtype = torch.float64)

    # I don't understand how the probabilities are supposed to work together with multibple objectives? why is objective weights even in here?
    for row in range(X.shape[0]):
        for col in range(X.shape[1]):
            probabilities[row] *= param_space.parameters[col].pdf(X[row, col].item())
...
def ei_bopro(
    ...
    **kwargs,
    ...
    if param_space.normalize_priors:
        good_prior_normalization_limits = estimate_prior_limits(param_space, settings["prior_limit_estimation_points"], objective_weights)
    else:
    ...
    # if we don't normalize the good_prior, for continous cases prior good can be larger than one.
    ...
    good_bad_ratios = torch.zeros(X.shape[0], dtype=torch.float64)
...
def ei_pibo(
    ...
    **kwargs,
    ...
    pibo_multipliers = prior #** (settings["model_weight"] / iteration_number)
