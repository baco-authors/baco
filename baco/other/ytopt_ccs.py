...
class YtOptRunner:
        ...
        for parameter in self.baco_space.parameters:
            if isinstance(parameter, RealParameter):
                ...
                cs.add_parameter(CCS.CategoricalParameter(
                    ...
                    default_index=(parameter.values.index(parameter.default) if parameter.default is not None else None)
                ))
            elif isinstance(parameter, OrdinalParameter):
                cs.add_parameter(CCS.OrdinalParameter(
                    name=parameter.name,
                    values=parameter.values,
                    default_index=(parameter.values.index(parameter.default) if parameter.default is not None else 0)
                ))
                ...
                cs.add_parameter(CCS.CategoricalParameter(
                    ...
                    default_index=(feasible_values.index(parameter.default) if parameter.default is not None else None)
                ))
    ...
    def run(self, configurations: List[Tuple[Any]]):
        ...
        data_array = self.baco_space.run_configurations(self.baco_mode, configurations_array, self.beginning_of_time, "ytopt_nonsense_file.csv", black_box_function=self.black_box_function, run_directory=self.run_directory)
    ...
    def main(self):
        ...
        # XX = self.optimizer.ask_initial(n_points=int(self.doe_samples))
