"""
THE CODE IN THIS FILE IS MORE OR LESS A FUNCTIONAL COPY OF THE DOE PART OF YTOPT (2022-05-01). YTOPT ALSO PRVIDES OTHER FEATURES SUCH AS ASYNCHRONUS SEARCH AND RUNNING MULTIPLE SETTINGS IN PARALLEL WHICH HAS BEEN REMOVED HERE.
"""
...
class YtOptRunner:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.number_of_cpus = settings["number_of_cpus"]
        self.application_name = settings["application_name"]
        set_output_data_file(settings, self.baco_space.all_names)
    ...
    def get_space(self):
        ...
        for parameter in self.baco_space.parameters:
            if isinstance(parameter, baco_space.RealParameter):
                params.append(
                    Real(
                        parameter.min_value,
                        parameter.max_value,
            ...
            elif isinstance(parameter, baco_space.IntegerParameter):
                params.append(
                    Integer(
                        parameter.min_value,
                        parameter.max_value,
            ...
            elif isinstance(parameter, baco_space.CategoricalParameter):
                params.append(
                    Categorical(
                        parameter.values,
                        name=parameter.name,
            ...
            elif isinstance(parameter, baco_space.OrdinalParameter):
                params.append(
                    Categorical(
                        parameter.values,
                        name=parameter.name,
            ...
            elif isinstance(parameter, baco_space.PermutationParameter):
                params.append(
                    Categorical(
                        parameter.filter_conditional_values({}),
                        name=parameter.name,
                    )
                )
    ...
    def run(self, configurations:List[Tuple[Any]]):
        ...
        if self.enable_feasible_predictor:
            ...
            for i, id in enumerate(feasible_indices):
                if not feasible_data_array.feasible_array[i].item():
                    feasible[id] = False
                    false_original_indices.append(id)
                else:
                    true_subindices.append(i)

            feasible_indices = [i for i in feasible_indices if not i in false_original_indices]
